...
Make sure you use the following dependency in your projet:
...
public DevServicesResultBuildItem createContainer() {
    ...
    return new DevServicesResultBuildItem.RunningDevService(FEATURE, container.getContainerId(), container::close, configOverrides)
        .toBuildItem();
}
----
